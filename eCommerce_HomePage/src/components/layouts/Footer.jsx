...
const Footer = () => {
  return (
    <footer className="bg-black/[0.8] text-gray-300 flex p-10 flex-wrap lg:justify-center lg:gap-[150px] gap-10 mt-10">
      <div className="flex flex-col">
        <h3 className="text-white m-1 font-bold">Online Shopping</h3>
        {footerLinksShop.map((item) => (
          <Link key={item.title} href={item.link} className="text-[14px] p-2">
            {item.title}
          </Link>
        ))}
      </div>
